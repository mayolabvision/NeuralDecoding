...
for m in models:
    ...
    if m == 4:
        ...
        model=DenseNNDecoder(units=[num_units,num_units],dropout=frac_dropout,num_epochs=n_epochs)
        model.fit(X_flat_train,y_train) 
        y_test_predicted=model.predict(X_flat_test) 
        mean_r2 = np.mean(get_R2(y_test,y_test_predicted))
        mean_rho = np.mean(get_rho(y_test,y_test_predicted))
        ...
        modelN=DenseNNDecoder(units=[num_units,num_units],dropout=frac_dropout,num_epochs=n_epochs)
        modelN.fit(X_flat_trainN,y_trainN) 
        y_test_predictedN=modelN.predict(X_flat_testN) 
        mean_r2N = np.mean(get_R2(y_testN,y_test_predictedN))
        mean_rhoN = np.mean(get_rho(y_testN,y_test_predictedN))
    ...
    if m == 5:
        ...
        model=SimpleRNNDecoder(units=num_units,dropout=frac_dropout,num_epochs=n_epochs)
        ...
        modelN=SimpleRNNDecoder(units=num_units,dropout=frac_dropout,num_epochs=n_epochs)
    ...
    if m == 6:
        ...
        model=GRUDecoder(units=num_units,dropout=frac_dropout,num_epochs=n_epochs)
        model.fit(X_train,y_train)
        y_test_predicted=model.predict(X_test)
        mean_r2 = np.mean(get_R2(y_test,y_test_predicted))
        mean_rho = np.mean(get_rho(y_test,y_test_predicted))
        ...
        modelN=GRUDecoder(units=num_units,dropout=frac_dropout,num_epochs=n_epochs)
        modelN.fit(X_trainN,y_trainN)
        y_test_predictedN=modelN.predict(X_testN)
        mean_r2N = np.mean(get_R2(y_testN,y_test_predictedN))
        mean_rhoN = np.mean(get_rho(y_testN,y_test_predictedN))
