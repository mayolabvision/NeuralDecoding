...
for n in mtfef_pairs:
    ...
    for i in foldneuron_pairs:
        ...
        results.append([s,i[1],i[0],n[0],n[1],mean_R2,mean_rho])
        params_all.append(max_params)
        neurons_all.append(neuron_inds)
        times_all.append(time_elapsed)

pfile = helpers.make_directory('neuron_sweep/')
with open(cwd+pfile+(jobname[0:18]+jobname[28:])+'.pickle','wb') as p:
    print(cwd+pfile+(jobname[0:18]+jobname[28:])+'.pickle')
